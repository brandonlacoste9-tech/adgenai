export class BrandVoiceEngine {
    ...
    trainingQueue = new Map();
    isTraining = false;
    async createBrandProfile(brandId, assets) {
    ...
    async validateBrandConsistency(brandId, creative) {
        ...
        try {
            ...
            const overallScore = Math.round((toneConsistency + visualAlignment + messagingAccuracy + complianceAdherence) / 4);
            const violations = this.identifyViolations({ toneConsistency, visualAlignment, messagingAccuracy, complianceAdherence }, brandProfile);
    ...
    async analyzeVoiceCharacteristics(assets) {
    ...
    async analyzeVisualIdentity(assets) {
    ...
    async extractMessagingFramework(assets) {
    ...
    async identifyComplianceRequirements(assets) {
    ...
    async trainBrandModel(brandId) {
        if (this.isTraining) {
            this.trainingQueue.set(brandId, this.brandProfiles.get(brandId)?.trainingData);
            return;
        }
    ...
    async analyzeToneConsistency(creative, profile) {
    ...
    generateImprovementSuggestions(violations, profile) {
        const suggestions = [];
    ...
    async updateBrandProfile(brandId, updates) {
        const profile = this.brandProfiles.get(brandId);
        if (!profile)
            throw new Error('Brand profile not found');
