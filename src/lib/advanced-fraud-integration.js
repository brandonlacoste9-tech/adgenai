export class HybridFraudDetectionEngine {
    providers = new Map();
    cache = new Map();
    realTimeMonitoring = new Map();
    constructor() {
        this.initializeFraudProviders();
        this.startRealTimeMonitoring();
    }
    initializeFraudProviders() {
    ...
    async analyzeWithHybridApproach(request) {
    ...
    combineProviderResults(results, request) {
        ...
        const overallRiskScore = Math.round(results.humanSecurity.riskScore * weights.humanSecurity +
    ...
    generateConsensusRecommendation(riskScore, agreementLevel, conflicts) {
    ...
    calculateHybridSavings(riskScore, budget) {
    ...
    generateHybridPreventionStrategies(riskScore, results) {
    ...
    identifyInternalThreats(riskScore) {
        if (riskScore > 50) {
            return ['Suspicious click patterns', 'Unusual geographic distribution'];
        }
        return ['Normal traffic patterns detected'];
    }
    generateInternalRecommendation(riskScore) {
        if (riskScore <= 25)
            return 'Low risk - continue campaign';
        if (riskScore <= 50)
            return 'Moderate risk - implement monitoring';
        return 'High risk - pause and investigate';
    }
    translateRecommendation(action) {
        const translations = {
        ...
        return translations[action] || 'Continue with caution';
    }
    ...
    getProviderFallback(providerId) {
        return {
            riskScore: 25,
            confidence: 0.75,
