export class AdvancedAttributionEngine {
    ...
    async recordAdvancedTouchpoint(touchpoint) {
    ...
    async generateAttributionReport(campaignIds, model, timeRange) {
        try {
    ...
    async calculateAdvancedAttribution(userJourneys, model) {
        const creativeResults = new Map();
        for (const [userId, journey] of userJourneys) {
            ...
            for (const conversion of conversions) {
                ...
                for (let i = 0; i < pathToConversion.length; i++) {
                    ...
                    if (weight < 1)
                        current.attribution.assistedConversions += weight;
    ...
    async calculateAttributionWeights(touchpoints, model) {
    ...
    async calculateDataDrivenWeights(touchpoints) {
    ...
    async calculateShapleyWeights(touchpoints) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let subset = 0; subset < Math.pow(2, n); subset++) {
                if (!(subset & (1 << i)))
                    continue; // Skip if player i not in subset
    ...
    async calculateMarkovWeights(touchpoints) {
    ...
    getChannelPerformanceMultiplier(platform) {
        const multipliers = {
            facebook: 1.0,
            instagram: 1.1,
            google: 1.2,
    ...
    calculateCoalitionValue(subset, touchpoints) {
    ...
    buildTransitionMatrix(channels) {
    ...
    calculateRemovalEffects(transitions, channels) {
    ...
    calculateConversionProbability(transitions, path) {
    ...
    generateAdvancedInsights(results, userJourneys) {
        const insights = [];
        // Revenue concentration analysis
        const topPerformer = results.reduce((max, current) => current.attribution.revenue > max.attribution.revenue ? current : max, results[0]);
        if (topPerformer) {
    ...
    generateStrategicRecommendations(results, insights) {
    ...
    async exportAttributionData(campaignIds, model, format) {
        const report = await this.generateAttributionReport(campaignIds, model, { start: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), end: new Date() });
        switch (format) {
            case 'csv':
